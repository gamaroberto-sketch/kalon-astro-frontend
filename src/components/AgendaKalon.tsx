    ...
    <div className="space-y-6 mt-8">
      ...
      {proximaJanela && (
        <div className="bg-[var(--astro-primary)]/10 border border-[var(--astro-primary)]/30 rounded-xl p-4 flex items-center justify-between">
          ...
          <div className="hidden sm:flex text-[var(--astro-primary)] text-3xl font-light">
            {proximaJanela.day}
          </div>
        </div>
      )}
                                ...
                                <div className="space-y-3">
                                  {campoInfo.auditoria.map((audItem, audIdx) => (
                                    <div key={audIdx} className="space-y-1">
                                      <strong className="block text-xs opacity-70">{audItem.titulo}</strong>
                                      {audItem.itens?.map((detalhe, detIdx) => (
                                        <div key={detIdx} className="text-xs flex justify-between border-b border-white/5 pb-1 mb-1 last:border-0 last:pb-0 last:mb-0">
                                          <span className="opacity-60">{detalhe.label}</span>
