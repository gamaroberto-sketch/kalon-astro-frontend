...
export default function KalonLegenda({ legenda }: KalonLegendaProps) {
  ...
  return (
    <div className="mt-8 space-y-4">
      <h3 className="text-sm uppercase tracking-wider font-semibold opacity-60 mb-3 border-b border-white/10 pb-2">
        Legenda de Ações
      </h3>
        ...
        {legenda.map((item, idx) => {
          ...
          return (
            <div key={idx} className="flex items-start gap-3 p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
              <div 
                className="flex items-center justify-center w-8 h-8 rounded-full bg-black/20 text-lg font-bold shrink-0 shadow-sm"
                style={{ color: cor, borderColor: `${cor}40`, borderWidth: '1px' }}
              >
                {item.icone}
              </div>
              <div className="flex-1">
                <strong className="block text-sm text-white/90 mb-1" style={{ color: cor }}>
                  {item.titulo}
                </strong>
